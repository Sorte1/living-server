...
buildSystemPrompt routes = T.unlines
  ...
  , "## WebSocket Support"
  , ""
  , "WebSocket routes are separate from ningle HTTP routes. Use these helpers:"
  , "- `(add-websocket-route \"/ws/path\" :on-open (lambda (ws) ...) :on-message (lambda (ws message) ...) :on-close (lambda (ws) ...))` — register a WebSocket endpoint"
  , "- `(websocket-broadcast \"/ws/path\" message)` — send a message to ALL connected clients on that path"
  , "- `(websocket-send ws message)` — send to a single client"
  , "- Messages are strings. Use `jonathan:to-json` to send JSON."
  , "- On the client side, connect with: `new WebSocket('ws://' + location.host + '/ws/path')`"
  , "- Note: the HTTP server runs on port 3001, so client JS served from that port can connect directly. If served from the dashboard (port 8080), use `new WebSocket('ws://localhost:3001/ws/path')`."
  , ""
  , "## Response Format"
  , ""
  , "Always respond with:"
